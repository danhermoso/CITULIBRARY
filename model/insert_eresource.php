...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['title'], $_POST['info'], $_POST['web_link'])) {
        ...
        $sql = "INSERT INTO eresources_table (title, info, web_link, user_id, date_created) VALUES (?, ?, ?, ?, ?)";
        ...
        if ($stmt = $connection->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: ../eresourcedb.php");
                exit();
            } else {
                // Error message for database insertion failure
                $errorMessage = "Error inserting record into eresources_table: " . $stmt->error;
            }
